...
class StatusColorTests(unittest.TestCase):
    ...
    def test_tor_active_is_green(self) -> None:
        self.assertIn("dot('torDot',s.tor_active?'green':'red')", self.source)

    def test_key_state_is_green_or_red(self) -> None:
        self.assertIn("dot('keyDot',s.key_ok?'green':'red')", self.source)

    def test_audio_busy_is_yellow(self) -> None:
        self.assertIn("dot('audioDot',s.audio_busy?'yellow':s.audio_ok?'green':'red')", self.source)

    def test_connection_semantics(self) -> None:
        expected = (
            "dot('linkDot',s.connected?'green':"
            "['starting','connecting','authenticating','listening','waiting','stopping']"
            ".includes(s.state)?'yellow':'red')"
        )
        self.assertIn(expected, self.source)

    def test_css_mapping_remains_unchanged(self) -> None:
